...
function TimeIt_user_view()
{
    ...
    $GETTemplate = FormUtil::getPassedValue('template', false, 'GETPOST');
    $catFilter = null;
    $cats = array();
    $shareFilter = null;
...
function TimeIt_user_subscribe($args=array())
{
	...
	{
		$result = pnModAPIFunc('TimeIt','subscribe','subscribe', array('eid'=>$id));
	} else 
	{
...
function TimeIt_user_viewUserOfSubscribedEvent($eid=false)
{
	...
	{
		$pnRender = pnRender::getInstance('TimeIt');
		$pnRender->assign('users', pnModAPIFunc('TimeIt','subscribe','userArrayForEvent',array('eid'=>$id)));
