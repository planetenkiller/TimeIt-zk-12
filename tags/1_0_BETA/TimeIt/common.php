...
class Timeit_common_createHandler
{
    ...
    function initialize(&$render)
    {
        ...
        $render->assign('addonItems', array(array('text'=>'---', 'value'=>'NONE'), array('text'=>'adress', 'value'=>'adress')));
        $this->addon = 'NONE';
        $render->assign('addonTPL', 'TimeIt_addon_'.$this->addon.'.htm');
    ...
    function handleCommand(&$render, &$args)
    {
      if ($args['commandName'] == 'create')
      {
        ...
        //print_r($dataForDB);exit();
      	...
      	WorkflowUtil::executeAction(pnModGetVar('TimeIt', 'workflow', 'standard'), $dataForDB, $data['wfactions'], 'TimeIt_events');
      	
      	// send an E-Mail?
      	if(pnModGetVar('TimeIt', 'notifyEvents'))
      	{
      		$pending = '';
      		$link = pnModUrl('TimeIt','user','event',array('id'=>$dataForDB['id']));
      		if($dataForDB['__WORKFLOW__']['schemaname'] == 'moderate' && $dataForDB['__WORKFLOW__']['state'] == 'waiting')
      		{
...
function TimeIt_templateWithTheme($render, $template)
{
