...
function TimeIt_calendarblock_display($blockinfo)
{
    ...
    if($processEvents) {
        ...
        $filterObj = new TimeIt_Filter();
        $filterObj->addGroup();
        if($vars['forUser']) {
            $filterObj->addExp('cr_uid:eq:-1');
            $filterObj->addExp('shareing:le:2');
        }
        if($vars['catFilter']) {
            $filterObj->addExp('category:in:'.(int)$catFilter[1]);
        }
        if($vars['filter']) {
            TimeIt_Filter::getFilterFormString($vars['filter'], $filterObj);
        }
        ...
        $events   =  pnModAPIFunc('TimeIt','user','getDailySortedEvents', array('preformat' =>true,
        ...
        if($vars['viewType'] == 'today') {
            $vars['maxEvents'] = 1000000; // big limit because we want to show all today events
        }
...
function TimeIt_calendarblock_update($blockinfo)
{
    ...
    $vars['filter'] =  FormUtil::getPassedValue('filter', null, 'POST');
