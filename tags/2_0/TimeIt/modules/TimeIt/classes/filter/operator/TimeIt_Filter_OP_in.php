 ...
 */
class TimeIt_Filter_OP_in extends TimeIt_Filter_OperatorIf
{
    ...
    public function __construct($field,$value) {
        ...
        if($field != 'category') {
            throw new InvalidArgumentException('The in operator "in" supports only "category" as field.');
        }
    ...
    {
        if(self::$filter) {
            return ($table?  $table.'.' : '').$this->table_columns[$this->field].' '.self::$filter;
        } else {
            return '';
        }
    }
