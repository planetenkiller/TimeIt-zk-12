...
function TimeIt_user_viewall()
{
    ...
    // data for pager
    $pnRender->assign('pager', array('numitems' => pnModAPIFunc('TimeIt', 'user', 'countGetAll',array('cid'=>$calendar['id'],'filter'=>$filter_obj)),
...
function TimeIt_user_new()
{
    ...
    return $render->pnFormExecute('TimeIt_user_new.htm', new TimeIt_common_createHandler('user'));
}
...
function TimeIt_user_feed()
{
    ...
    $GETMODE = FormUtil::getPassedValue('mode', 'last', 'GET');
    ...
    if($GETMODE == 'last') {
        ...
        $filster_sql = $filter_obj->toSQL();
        if($filster_sql) {
            $where .= ' AND '.$filster_sql;
        }

        // create class and load events
        $class = new $class();
        $array = $class->get($where, $cols['lu_date'].' DESC', -1, (int)pnModGetVar('TimeIt', 'rssatomitems'));
...
function TimeIt_user_viewUserOfSubscribedEvent($args=array())
{
    ...
    $dheobj = DBUtil::selectObjectByID('TimeIt_date_has_events', $id);
    $obj = pnModAPIFunc('TimeIt','user','get',array('id'=>($dheobj['localeid']? $dheobj['localeid'] : $dheobj['eid'])));
...
function TimeIt_user_rruleTest3()
{
    pnModAPILoad('TimeIt','user'); // load api
    
    
    $obj = array();
    $obj['startDate'] = '2008-09-21';
    $obj['endDate'] = '2008-09-21';
        $obj['repeatType'] = 1;
    $obj['repeatSpec'] = 'year';
        $obj['repeatFrec'] = 1;

        $out = "ret:<br />";
    $rep = new TimeitRepeatProzessor($obj);
        $ret = $rep->doInsertArray();
        foreach ($ret as $row) 
    {
        $out .= $row.'<br />';
    }
