...
function TimeIt_userapi_getDailySortedEvents($args)
{
    ...
    $sql = 'pn_status = 1 AND a.cid = '.DataUtil::formatForStore((int)$args['cid']).' ';
    ...
    $sql .= ' AND a.the_date >= \''.DataUtil::formatForStore($start).'\' AND a.the_date <= \''.DataUtil::formatForStore($end).'\'';
    ...
    if(!isset($args['filter_obj']) || !$args['filter_obj']->hasFilterOnField('sharing')) {
        $sql .= ' AND (';
        if(!empty($User_ID)) {
            $sql .= '(('.$column['sharing'].' = 1 OR '.$column['sharing'].' = 2)  AND pn_cr_uid = '.DataUtil::formatForStore(pnUserGetVar('uid')).') OR ';
        }

        $sql .= $column['sharing'].' = 3 OR '.$column['sharing'].' = 4)';
    }

    if(isset($args['filter_obj']) && $args['filter_obj'] instanceof TimeIt_Filter) {
        $filter_sql = $args['filter_obj']->toSQL();
...
function TimeIt_userapi_getAll($args)
{
    ...
    // load event form DB
    $array = $class->events($where, null, $args['startnum']-1, $args['numitems']);
    $ret = array();
    //print_r($array);exit();
...
function TimeIt_userapi_countGetAll($args)
{
    ...
    $sql = "SELECT COUNT(DISTINCT pn_id)
            ...
            RIGHT JOIN ".$t['TimeIt_date_has_events']." a
                ON ((a.eid = tbl.pn_id AND a.localeid IS NULL)
                     OR (a.localeid IS NOT NULL AND a.localeid = tbl.pn_id)
                   )";
...
function TimeIt_privuserapi_addEventToArray(&$array, $tmestamp, $obj)
{
    ...
    if(!isset($array[$tmestamp][$catID])) {
            ...
            $array[$tmestamp][$catID]['info'] = array('name'=>$name,'color'=>$obj['__CATEGORIES__'][$property]['__ATTRIBUTES__']['color']);
        ...
        if(isset($args['obj']['__WORKFLOW__'])) {
            ...
            if(!isset($args['obj']['__META__']['TimeIt']['recurrenceOnly']) || !$args['obj']['__META__']['TimeIt']['recurrenceOnly']) {
                if(!isset($args['noRecurrenceCalculation']) || !$args['noRecurrenceCalculation']) {
                    $prozi = new TimeitRepeatProzessor($args['obj']);
                    $prozi->doInsert();
                }
...
function TimeIt_userapi_updateRecurrences($args)
{
    ...
    if($obj['repeatType'] == $master['repeatType'] && $obj['repeatSpec'] == $master['repeatSpec'] && $obj['repeatFrec'] == $master['repeatFrec'] && $obj['startDate'] == $master['startDate']) {
        if($obj['endDate'] < $master['endDate']) {
            ...
            $prozi = new TimeitRepeatProzessor($temp);
            $array = $prozi->doInsertArray();
    ...
    } else if($obj['startDate'] != $master['startDate'] || $obj['repeatType'] == 2) {
        pnModAPIFunc('TimeIt','user','deleteAllRecurrences', array('obj'=>$obj));
        TimeIt_userapi_prozessRepeat($obj, true);
        $prozi = new TimeitRepeatProzessor($temp);
        $prozi->doInsert();
    }
}
...
function TimeIt_userapi_deleteAllRecurrences($args)
{
    if(!isset($args['obj']) || empty($args['obj'])) {
        ...
        $dhearray = DBUtil::selectObjectArray('TimeIt_date_has_events', 'eid = '.(int)$args['obj']['id']);
...
{
    // use new Version
    return TimeIt_userapi_arrayForMonthViewV2($args);
}
...
function TimeIt_userapi_anonymizeEventsOfUser($args)
{
    ...
    // anonymize the cr_uid of all these events
    $obj = array('cr_uid'=>1);
...
class TimeitRepeatProzessor
{
    var $obj;
    var $ignoreFirst;
    var $updateFirst;
    var $noEndDateForFirst;
    var $intoArray;
    
    var $hasMasterEventToUpdate;
    ...
    function TimeitRepeatProzessor(&$obj)
    {
        $this->obj =& $obj;
        $this->ignoreFirst = false;
        $this->noEndDateForFirst = false;
        $this->updateFirst = false;
        $this->intoArray = array();
        $this->hasMasterEventToUpdate = false;
        $this->mmidSet = false;
        $this->master_idSet = false;
        $this->repeatIrg = explode(',', $this->obj['repeatIrg']);
    }
    
    function setIgnoreFirst($b)
    {
        $this->ignoreFirst = $b;
        ...
        {
            $this->hasMasterEventToUpdate = true;
        }
    }
    
    function setNoEndDateForFirst($b)
    {
        $this->noEndDateForFirst = $b;
    }
    
    function setUpdateFirst($b)
    {
        $this->updateFirst = $b;
    }
    
    function doInsert($startT=null, $endT=null)
    {
        $this->doWork(null,$startT, $endT);
    }
    
    function doInsertArray($startT=null, $endT=null)
    {
        $this->doWork(true, $startT, $endT);
        return $this->intoArray;
    }
    
    // private functions
    function doWork($dointoArray=NULL,$startT=null,$endT=null)
    {
        $intoArray = $dointoArray;
        $obj =& $this->obj;
        $ignoreFirest = $this->ignoreFirst;
        $start = ($startT!=null)? $startT: $obj['startDate'];
        $end = ($endT!=null)? $endT: $obj['endDate'];
        $this->schema = 'standard';//$obj['_WORKFLOW_']['TIschemaname'];
        $this->state =  'submit';//$obj['_WORKFLOW_']['TIAction'];
        $obj['___NOREPEAT___'] = true;
        ...
        if($obj['repeatType'] == 0)
        {
            ...
            if($obj['endDate'] == $obj['startDate'])
            {
                ...
                if($ignoreFirest)
                {
                    ...
                    if($intoArray == null)
                      $this->insert(DateUtil::parseUIDate($obj['startDate']));
                    else
                      $this->insertInToArray(DateUtil::parseUIDate($obj['startDate']), $intoArray);
                }
            } else
            {
                ...
                for($i=0;$i<=$diff['d'];$i++)
                {
                    ...
                    if($ignoreFirest)
                    {
                        ...
                        if($intoArray == null)
                            ...
                            $this->insertInToArray($timestamp, $intoArray);
                    }
                    $timestamp += 86400;
                }
            }
        } else if($obj['repeatType'] == 1)
        {
                    ...
                    if($date >= $start && $date <= $end && $date >= $obj['startDate'])
                    {
                        ...
                        if($ignoreFirest)
                        {
                            ...
                            if($intoArray == null)
                                ...
                                $this->insertInToArray($temp, $intoArray);
                        }
                    }
                    $weeks++;
                    $date = DateUtil::getDatetime(strtotime('+'.($weeks*$obj['repeatFrec']).' week', strtotime($obj['startDate'])), _DATEINPUT);
            ...
            } else if($obj['repeatSpec'] == "year")
            {
                ...
                for($year=$years_start; $year <= $years_ende; $year += $obj['repeatFrec']) {
                    ...
                    $date = DateUtil::getDatetime($temp, _DATEINPUT);
                    if($date >= $start && $date <= $end && $date >= $obj['startDate']) {
                        if($ignoreFirest)
                        {
                            $ignoreFirest = false;
        ...
        } else if($obj['repeatType'] == 2)
        {
            ...
            foreach($specOld[0] AS $specValue)
            {
                ...
                $stamp = strtotime($a.' '.$b.$c, strtotime('-1 day',$date));
                
                if((int)$spec[0] == 2)
                {
                    $stamp = strtotime('+7 day', $stamp);
                    ...
                    if($date >= $start && $date <= $end && $date >= $obj['startDate'])
                    {
                        ...
                        if($ignoreFirest)
                        {
                            ...
                            if($intoArray == null)
                                ...
                                $this->insertInToArray($temp1, $intoArray);
                        }
                    }
        ...
        } else if($obj['repeatType'] == 3)
        {
            if($ignoreFirest)
            {
                ...
                if($intoArray == null)
                    $this->insert(strtotime($obj['startDate']));
                else
                    $this->insertInToArray(strtotime($obj['startDate']), $intoArray);
            }
            ...
            foreach($dates AS $date)
            {
                if($ignoreFirest)
                {
                    ...
                    if($intoArray == null)
                        $this->insert(strtotime($date));
                    else
                        $this->insertInToArray(strtotime($date), $intoArray);
                }
            }
        } else if($obj['repeatType'] == 4)
        {
            $ret = array();
            ...
            // add all recurrences
            foreach($ret AS $timestamp => $unused) {
                if($ignoreFirest) {
                    $ignoreFirest = false;
                } else {
                    if($intoArray == null)
    ...
    {
        $insert = array();
        $insert['eid'] = $this->obj['id'];
        $insert['date'] = DateUtil::getDatetime($timestamp, _DATEINPUT);
        $insert['cid'] = $this->obj['cid'];
        
        // is this date in the irgnore dates array?
        if(in_array($insert['date'], $this->repeatIrg)) {
            return; // ignore
        }
    ...
    function insertInToArray($timestamp, $array)
    {              
        $insert = $this->obj;
        $insert['startDate'] = DateUtil::getDatetime($timestamp, _DATEINPUT);
        
        // is this date in the irgnore dates array?
        if(in_array($insert['startDate'], $this->repeatIrg)) {
            return; // ignore
        }
        
        $this->intoArray[] = $insert['startDate'];
    }
}
