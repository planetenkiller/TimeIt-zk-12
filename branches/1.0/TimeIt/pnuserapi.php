...
function TimeIt_userapi_getDailySortedEvents($args)
{
    ...
    $shareFilter = (isset($args['shareFilter']))? $args['shareFilter']: null;
        ...
        if(!empty($shareFilter) && ((int)$shareFilter == 1 || (int)$shareFilter == 2))
        {
        	$sql .= ' AND '.$column['sharing'].' = '.DataUtil::formatForStore($shareFilter).'';
			if(!empty($User_ID))
			{
        ...
        {
        	$sql .= ' AND ((('.$column['sharing'].' = 2 OR '.$column['sharing'].' = 1)';
			if(!empty($User_ID))
			{
        		$sql .= ' AND pn_cr_uid = '.DataUtil::formatForStore(pnUserGetVar('uid'));
			}
			
			$sql .= ') OR '.$column['sharing'].' = 3)';
        }
