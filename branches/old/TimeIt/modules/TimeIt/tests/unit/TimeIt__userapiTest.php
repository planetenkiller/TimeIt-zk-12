...
class TimeIt__userapiTest extends ZkUnitTestCase
{
    ...
    public function testgetDHE_withObj()
    {
        $obj = array('id' => $this->sharedFixture->TimeIt_events['one']['id']);
        $dhe_obj = pnModAPIFunc('TimeIt','user','getDHE', array('obj'=>$obj));

        $this->assertNotNull($dhe_obj);
        $this->assertTrue($dhe_obj != false);
        $this->assertEquals($this->sharedFixture->TimeIt_events['one']['id'], $dhe_obj['eid']);
        $this->assertEquals('2009-01-01', $dhe_obj['date']);
}

    public function testgetAll()
    {
        $rows = pnModAPIFunc('TimeIt','user','getAll',array('cid'=>$this->sharedFixture->TimeIt_calendars['one']['id']));

        $this->assertEquals(15, count($rows));
        $this->assertEquals($this->sharedFixture->TimeIt_events['one']['title'], $rows[0]['title']);
    }

    public function testgetAll_withFilter()
    {
        if(!class_exists('TimeIt_Filter')) {
            Loader::loadFile('Filter.class.php','modules/TimeIt/classes/filter');
        }
        $filter_obj = new TimeIt_Filter();
        $filter_obj->addGroup()
          ->addExp('startDate:eq:2009-01-01');
        $rows = pnModAPIFunc('TimeIt','user','getAll',array('cid'=>$this->sharedFixture->TimeIt_calendars['one']['id'],'filter_obj'=>$filter_obj));

        $this->assertEquals(2, count($rows));
        $this->assertEquals($this->sharedFixture->TimeIt_events['one']['id'], $rows[0]['id']);
        $this->assertEquals($this->sharedFixture->TimeIt_events['two']['id'], $rows[1]['id']);
    }

    public function testcountGetAll()
    {
        $this->assertEquals(15, pnModAPIFunc('TimeIt','user','countGetAll',array('cid'=>$this->sharedFixture->TimeIt_calendars['one']['id'])));
    }

    public function testcountGetAll_withFilter()
    {
        $filter_obj = new TimeIt_Filter();
        $filter_obj->addGroup()
          ->addExp('startDate:eq:2009-01-01');

        $this->assertEquals(2, pnModAPIFunc('TimeIt','user','countGetAll',array('filter_obj'=>$filter_obj,'cid'=>$this->sharedFixture->TimeIt_calendars['one']['id'])));
    }
}
