...
class SyncML_Command_Map extends SyncML_Command {
    ...
    var $_mapItems = array();
    var $_mapItems_currentIndx = 0;
    var $_mapItems_sourceOk = false;
    var $_mapItems_targetOk = false;
    ...
    function endElement($uri, $element)
    {
        ...
        case 4:
            if ($element == 'LocURI') {
                ...
                if($this->_mapItems_sourceOk && $this->_mapItems_targetOk) {
                    $this->_mapItems_currentIndx++;
                    $this->_mapItems_sourceOk = false;
                    $this->_mapItems_targetOk = false;
                }
                if($this->_stack[2] == 'Source') {
                    $this->_mapItems_sourceOk = true;
                    $this->_mapItems[$this->_mapItems_currentIndx][0] = $this->_mapSource;
                } else if($this->_stack[2] == 'Target') {
                    $this->_mapItems_targetOk = true;
                    $this->_mapItems[$this->_mapItems_currentIndx][1] = $this->_mapTarget;
                }
            }
            break;
        }
    ...
    function handleCommand($debug = false)
    {
        if (!$debug && $this->_mapItem) {
            ...
            if (!$state->authenticated) {
                ...
                foreach($this->_mapItems AS $item) {
