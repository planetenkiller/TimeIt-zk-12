...
require_once dirname(__FILE__).'/Device/Horde/String.php';
...
class SyncML_Backend {
    ...
    function factory($driver, $params = null)
    {
        ...
        if (basename($driver) == $driver) {
            include_once dirname(__FILE__).'/Backend/' . $driver . '.php';
        } else {
                          ...
                          $backendMode = SYNCML_BACKENDMODE_SERVER)
    {
        ...
        if ($this->_backendMode == SYNCML_BACKENDMODE_SERVER) {
            ...
            if($GLOBALS['_PNSession']['obj']) {
                session_id($sid);
                $GLOBALS['_PNSession']['obj']['sessid'] = session_id();
            }
        }
    }
