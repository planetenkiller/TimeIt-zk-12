...
class HTTP_WebDAV_Server 
{
    ...
    */
    ...
    function http_OPTIONS() 
    {
        ...
        header("DAV: "  .join(", ", $dav).', calendar-access');
                ...
                foreach ($file["props"] as $key => $prop) {
                    ...
                    if (!isset($prop["val"]) || $prop["val"] === "" || $prop["val"] === false) {
                        ...
                        case "getlastmodified":
                            ...
                            break;
                        /*case "resourcetype":
                            echo "     <D:resourcetype><D:collection/><ns1:$prop[val]/></D:resourcetype>\n";
                            break;*/
                        case "supportedlock":
    ...
    function _prop2xml($prop, $ns_hash)
    {
        ...
        if ($prop["ns"]) {
            ...
            if($prop['attr'] && is_array($prop['attr'])) {
                foreach($prop['attr'] AS $key => $val) {
                    $res .= ' '.$key.'="'.$this->_prop_encode(htmlspecialchars($val)).'"';
                }
            }
        ...
        if(!empty($prop['val'])) {
            if (is_array($prop['val'])) {
                if(isset($prop['val']['name']) && isset($prop['val']['val'])) {
                    $res .= $this->_prop2xml($prop['val'], $ns_hash);
                } else {
                    foreach($prop['val'] AS $subprop) {
                        $res .= $this->_prop2xml($subprop, $ns_hash);
                    }
                }
