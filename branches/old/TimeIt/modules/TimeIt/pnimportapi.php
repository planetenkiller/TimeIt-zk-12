...
//           exit();
    ...
    /**
     * The Method doImport() calls this function before it inserts the new event.
     * This method allows subclasses to modify the event before its creation.
     */
...
class TimeItImportApiInputPC
{
    ...
    function TimeItImportApiInputPC($prefix)
    {
        ...
        $this->firstCall = true;
        $this->maxDate = DateUtil::getDatetime(strtotime('+1 year',time()), _DATEINPUT);
...
class TimeItImportApiInputICal
{
    ...
    function TimeItImportApiInputICal($path, $isString=false)
    {
        $this->path = $path;
    ...
    function check()
    {
        if(!$this->isString && !file_exists($this->path))
        {
    ...
    function load()
    {
        ...
        if(!$this->isString) {
            ...
            $this->data->parse(false, $this->path);
        }
    ...
    function startDate()
    {
        ...
        return DateUtil::getDatetime($startDate, _DATEINPUT);
    }
    ...
    function endDate()
    {
        if($this->repeatType() == 4)
        {
            ...
            if(isset($rrule['UNTIL']))
            {
                ...
                $endDate = DateUtil::getDatetime($endDate, _DATEINPUT);
                return $endDate;
            } else
            {
        ...
        if($property)
        {
            ...
            } else
            {
                ...
                return DateUtil::getDatetime(strtotime('-1 day', strtotime($endDate)), _DATEINPUT);
            }
        } else 
        {
            return $this->startDate();
