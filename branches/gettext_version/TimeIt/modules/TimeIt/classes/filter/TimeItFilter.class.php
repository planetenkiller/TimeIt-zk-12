...
class TimeItFilter
{
    ...
    /**
     * Add a expression group. All expressions in the same group are linked with AND.
        ...
        if($op) {
            ...
            if($this->objectType == 'event') {
                ...
                if($op->getField() == 'sharing' && (
                    ...
                    $op2 = TimeIt_Filter_OperatorIf::operatorFromExp($this->objectType, 'cr_uid:eq:-1');
                    if($op2 != null) {
    ...
    {
        $ret = new TimeItFilter($objectType);
     ...
     * @return TimeIt_Filter created TimeIt_Filter
     */
    public static function getFilterFormString($string, TimeIt_Filter $filter=null, $objectType='event')
    {
        if($filter) {
            $ret = $filter;
        } else {
            $ret = new TimeIt_Filter($objectType);
        }
        if(!$ret->hasGroup()) {
            $ret->addGroup();
        }
