...
function eventEditInitJS()
{
    if($('allDay').checked) {
        $('timeit_form_time').hide();
    }

    $('allDay').observe('click', function() {
        $('timeit_form_time').toggle();
    });

    Event.observe($('group'), 'change', groupEditOnChange);
}

/**
 * De-select the all option if a non-all option is selected.
 */
function groupEditOnChange()
{
    var dropdown       = $('group');
    var nonAllSelected = false;
